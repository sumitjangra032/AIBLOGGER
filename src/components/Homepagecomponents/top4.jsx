...
export default function Top4() {
  ...
  useEffect(() => {
    const fetchTrending = async () => {
      const q = query(collection(db, "blogs"), orderBy("views", "desc"), limit(8));
      try {
        ...
        <div className="space-y-1 w-full max-w-md mx-auto mt-5">
          {categories.map((cat, index) => (
            <div key={index} className="relative h-20 rounded-xl overflow-hidden group">
              ...
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-white text-sm font-semibold tracking-wide px-3 py-1 bg-white/20 rounded-xl backdrop-blur-sm">
                {cat.name}
              </span>

              <Link to={`/category/${cat.name.toLowerCase()}`} className="absolute inset-0">
          ...
          <div className="space-y-3">
            ...
            {popularPosts.map((post) => (
              <div key={post.id} className="grid grid-cols-1 lg:grid-cols-3 gap-2 mt-2">
                ...
                <div className="lg:col-span-2 border-b border-gray-300 hover:bg-red-50">
